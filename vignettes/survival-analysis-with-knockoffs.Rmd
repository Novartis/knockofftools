...
```{r, eval=FALSE}
set.seed(123)
W <- knockoff.statistics(y, X, type="survival", M=31)
head(W)
```
...
plot(S)
```
By default the stable variable names will be colored in red. Note the message "Co-Clustering successfully terminated!". We apply co-clustering of the rows and columns of the heatmap with the `blockcluster` package, which identifies blocks of similarities in the binary selections. We then order the blocks according to increasing mean number of selections. This helps with aesthetics, but also helps visualize the most important variables that should tend towards the top of the heatmap. The plotting function will order the heatmap rows/columns according to importance only if the `blockcluster` and `dplyr` packages are installed.
