...
```{r, message=TRUE}
W <- knockoff.statistics(y=y, X=X, type="regression", statistic = "stat_predictive_glmnet", trt=trt, M=1)
...
```{r, message=TRUE}
W <- knockoff.statistics(y=y, X=X, type="regression", statistic = "stat_predictive_causal_forest", trt=trt, M=1)
S = variable.selections(W, level = 1, error.type="pfer")
S$stable.variables
```
