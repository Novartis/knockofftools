...
W <- knockoff.statistics(y, X, M=31)
head(W)
```
...
plot(S)
```
By default the stable variables will be colored in red. We apply co-clustering of the rows and columns of the heatmap and then order the blocks according to increasing mean number of selections. This helps with aesthetics, but also helps visualize the most important variables that should tend towards the top of the heatmap.
